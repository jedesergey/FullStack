...
session_start();
if (!isset($_SESSION['SkrillPayeer'])) {    
$_SESSION['SkrillPayeer'] = 0; 
}
...
if (@$_REQUEST['hourly']) {
$_SESSION['SkrillPayeer'] = 0;    
header("Location: skrillpayeer.php");

}

if (@$_REQUEST['daily']) {
$_SESSION['SkrillPayeer'] = 1;    
header("Location: skrillpayeer.php");
...
<script src="../realorders/real4.js"></script>
...
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="b4" onclick="buy(this.id)" class="button center">Yes</a></div>
...
<h1 style="color: #c92d2d; margin: 10px;" class="tcenter">You really want to create the order?</h1>
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="s4" onclick="sell(this.id)" class="button center">Yes</a></div>
...
<div id="orders-stats">
<div class="orderStats">Last Price: <strong>282.9498 Payeer</strong></div>
<div class="orderStats">Low: <strong>266.1757 Payeer</strong></div>
<div class="orderStats">High: <strong>284.29 Payeer</strong></div>
<div class="clear"></div>
<div class="orderStats">Volume: <strong>282 Skrill / 77920 Payeer</strong></div>
...
<li class="pairs-selected">Skrill/Payeer<span style="display:block">0.364</span></li>
...
<h1 id="n0">Skrill/Payeer</h1></div>
<form action="skrillpayeer.php" method="post">
...
<p><a href="javascript:void(0)" onclick="a_calc(19)"><b><span id="cur1" class="money_Payeer">0</span><input id="cur1_full" value="0" type="hidden"> Payeer</b></a></p>
</div>
<div style="width: 130px; float: right;">Lowest ask Price
<p><b><span id="min_price">0</span> Payeer</b></p>
</div>
<div class="clear"></div>
</div>
...
<td>
<input id="b_price" style="width: 110px;" maxlength="8" value="0"  type="text"> Payeer</td>
</tr>
<tr>
<td>Total:</td>
<td><b id="b_all">0</b> <b>Payeer</b></td>
</tr>
<tr>
<td>Fee:</td>
<td><b id="b_fee">0</b> <b>Payeer</b></td>
</tr>
...
$red = mysql_query("SELECT * FROM ordersSkrillPayeer WHERE action='0' AND type='sell' AND time < '$date'");
...
$red = mysql_query("SELECT * FROM realSkrillPayeer WHERE action='0' AND type='sell' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
<p><b><span id="max_price">0</span> Payeer</b></p>
</div>
<div class="clear"></div>
</div>
<table class="inblock tabla2" style="width: 100%;">
...
<td>Price per Skrill:</td>
<td>
<input id="s_price" style="width: 110px;" maxlength="8" value="0"  type="text"> Payeer</td>
</tr>
<tr>
<td>Total:</td>
<td><b id="s_all">0</b> <b>Payeer</b></td>
</tr>
<tr>
<td>Fee:</td>
<td><b id="s_fee">0</b> <b>Payeer</b></td>
</tr>
...
<p class="gray" style="text-align: right;">Total: 215365.44 Payeer</p>
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersSkrillPayeer WHERE action='0' AND type='buy' AND time < '$date'");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$red = mysql_query("SELECT * FROM realSkrillPayeer WHERE action='0' AND type='buy' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$user = $_SESSION['user'];
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM realSkrillPayeer WHERE action='0' AND user='$user' AND time < '$date' ORDER BY time DESC");
while ($row = mysql_fetch_array($red)){  
$price = $row['price'];
$amount = $row['amount'];
$total = $row['total'];
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersSkrillPayeer WHERE action='1' AND time2 <= '$date' ORDER BY time2 DESC");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
...
$red = mysql_query("SELECT * FROM realSkrillPayeer WHERE action='1'");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
