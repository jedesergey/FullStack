...
session_start();
if (!isset($_SESSION['CocaPayMonero'])) {    
$_SESSION['CocaPayMonero'] = 0; 
}
...
if (@$_REQUEST['hourly']) {
$_SESSION['CocaPayMonero'] = 0;    
header("Location: cocapaymonero.php");

}

if (@$_REQUEST['daily']) {
$_SESSION['CocaPayMonero'] = 1;    
header("Location: cocapaymonero.php");
...
<script src="../realorders/real19.js"></script>
...
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="b19" onclick="buy(this.id)" class="button center">Yes</a></div>
...
<h1 style="color: #c92d2d; margin: 10px;" class="tcenter">You really want to create the order?</h1>
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="s19" onclick="sell(this.id)" class="button center">Yes</a></div>
...
<div id="orders-stats">
<div class="orderStats">Last Price: <strong>282.9498 CocaPay</strong></div>
<div class="orderStats">Low: <strong>266.1757 CocaPay</strong></div>
<div class="orderStats">High: <strong>284.29 CocaPay</strong></div>
<div class="clear"></div>
<div class="orderStats">Volume: <strong>282 Monero / 77920 CocaPay</strong></div>
...
<li><a href="/exchange/cocapayokpay.php">CocaPay/OKPay<span style="display:block">0.00977</span></a></li>
<li><a href="/exchange/cocapaypayeer.php">CocaPay/Payeer<span style="display:block">307.856</span></a></li>
<li class="pairs-selected">CocaPay/Monero<span style="display:block">0.364</span></li>
<div class="clear"></div>
</ul>
...
<h1 id="n0">CocaPay/Monero</h1></div>
<form action="cocapaymonero.php" method="post">
...
<p><a href="javascript:void(0)" onclick="a_calc(19)"><b><span id="cur1" class="money_Monero">0</span><input id="cur1_full" value="0" type="hidden"> Monero</b></a></p>
</div>
...
<input id="b_CocaPay" style="width: 110px;" maxlength="25" value="0" onkeyup="sendbuy(this.value)" type="text">
</td>
</tr>
<tr class="price-row">
...
<p class="gray" style="text-align: right;">Total: 551.42 CocaPay</p>
...
$red = mysql_query("SELECT * FROM ordersCocaPayMonero WHERE action='0' AND type='sell' AND time < '$date'");
...
$red = mysql_query("SELECT * FROM realCocaPayMonero WHERE action='0' AND type='sell' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
<td style="width: 150px;">Amount CocaPay:</td>
<td style="width: 153px;">
<input id="s_CocaPay" style="width: 110px;" maxlength="25" value="0" onkeyup="sendsell(this.value)" type="text">
</td>
</tr>
<tr class="price-row">
<td>Price per CocaPay:</td>
...
<p class="gray" style="text-align: right;">Total: 215365.44 Monero</p>
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersCocaPayMonero WHERE action='0' AND type='buy' AND time < '$date'");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$red = mysql_query("SELECT * FROM realCocaPayMonero WHERE action='0' AND type='buy' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
<th style="width:110px">Amount CocaPay</th>
<th style="width:110px">Total Monero</th>
...
$user = $_SESSION['user'];
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM realCocaPayMonero WHERE action='0' AND user='$user' AND time < '$date' ORDER BY time DESC");
while ($row = mysql_fetch_array($red)){  
$price = $row['price'];
$amount = $row['amount'];
$total = $row['total'];
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersCocaPayMonero WHERE action='1' AND time2 <= '$date' ORDER BY time2 DESC");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
...
$red = mysql_query("SELECT * FROM realCocaPayMonero WHERE action='1'");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
...
var amountbuy = document.getElementById('b_CocaPay').value;
...
function sendsell(){
var amountsell = document.getElementById('s_CocaPay').value;
