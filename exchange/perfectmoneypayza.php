...
session_start();
if (!isset($_SESSION['PerfectMoneyPayza'])) {    
$_SESSION['PerfectMoneyPayza'] = 0; 
}
...
if($_SESSION['PerfectMoneyPayza'] == 1){
$dis2 = 'disabled="disabled"'; 
$ch1 = 'PerfectMoneyPayza';
$interval = '1 days';
$format = '%b %e';
$mintime = $minday; 
$maxtime = $maxday;    
...
$ch1 = 'ohlcordersPerfectMoneyPayza';
$interval = '1 hours';
$format = '%R';
...
if (@$_REQUEST['hourly']) {
$_SESSION['PerfectMoneyPayza'] = 0;   
header("Location: perfectmoneypayza.php");

}

if (@$_REQUEST['daily']) {
$_SESSION['PerfectMoneyPayza'] = 1;   
header("Location: perfectmoneypayza.php");
...
<img src='../images/1px.png' class='main-s main-s-dollar_small' alt='dollar_small'/> <b class='money_Payza'>0</b> Payza
</div>
...
<title>PerfectMoney/Payza</title>
...
<script src="../realorders/real7.js"></script>
...
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="b7" onclick="buy(this.id)" class="button center">Yes</a></div>
...
<h1 style="color: #c92d2d; margin: 10px;" class="tcenter">You really want to create the order?</h1>
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="s7" onclick="sell(this.id)" class="button center">Yes</a></div>
...
<div id="orders-stats">
<div class="orderStats">Last Price: <strong>0.861 Payza</strong></div>
<div class="orderStats">Low: <strong>0.822 Payza</strong></div>
<div class="orderStats">High: <strong>0.884 Payza</strong></div>
<div class="clear"></div>
<div class="orderStats">Volume: <strong>12208 PerfectMoney / 10358 Payza</strong></div>
<div class="orderStats">Server Time: <strong>29.10.15 18:43</strong></div>
<div class="clear"></div>
</div>
</div>
<? echo $noauth; ?>
...
<li class="pairs-selected">PerfectMoney/Payza<span style="display:block">0.364</span></li>
...
<h1 id="n0">PerfectMoney/Payza</h1></div>
<form action="perfectmoneypayza.php" method="post">
...
<h1>Buy PerfectMoney</h1>
<input id="pair" value="29" type="hidden">
<div class="inblock order_header">
<div style="width: 130px; float: left;">Your balance:
<p><a href="javascript:void(0)" onclick="a_calc(29)"><b><span id="cur1" class="money_Payza">0</span><input id="cur1_full" value="0" type="hidden"> Payza</b></a></p>
</div>
...
<a href="javascript:void(0)" onclick="noticebuy()" class="button" style="float: right;">Buy PerfectMoney</a>
...
<p class="gray" style="text-align: right;">Total: 87540.06 PerfectMoney</p>
...
$red = mysql_query("SELECT * FROM ordersPerfectMoneyPayza WHERE action='0' AND type='sell' AND time < '$date'");
...
$red = mysql_query("SELECT * FROM realPerfectMoneyPayza WHERE action='0' AND type='sell' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
<h1>Sell PerfectMoney</h1>
<input id="pair" value="29" type="hidden">
<div class="inblock order_header">
<div style="width: 130px; float: left;">Your balance:
<p><a href="javascript:void(0)" onclick='all_money("cur2_full", "s_Monero")'><b><span id="cur2" class="money_PerfectMoney">0</span><input id="cur2_full" value="0" type="hidden"> PerfectMoney</b></a></p>
</div>
...
<a href="javascript:void(0)" onclick="noticesell()" class="button" style="float: right;">Sell PerfectMoney</a>
...
<p class="gray" style="text-align: right;">Total: 8537.11 Payza</p>
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersPerfectMoneyPayza WHERE action='0' AND type='buy' AND time < '$date'");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$red = mysql_query("SELECT * FROM realPerfectMoneyPayza WHERE action='0' AND type='buy' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$user = $_SESSION['user'];
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM realPerfectMoneyPayza WHERE action='0' AND user='$user' AND time < '$date' ORDER BY time DESC");
while ($row = mysql_fetch_array($red)){  
$price = $row['price'];
$amount = $row['amount'];
$total = $row['total'];
...
<th width="20%">Amount (PerfectMoney)</th>
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersPerfectMoneyPayza WHERE action='1' AND time2 <= '$date' ORDER BY time2 DESC");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
...
$red = mysql_query("SELECT * FROM realPerfectMoneyPayza WHERE action='1'");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
