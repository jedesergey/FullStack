...
session_start();
if (!isset($_SESSION['CocaPayPayza'])) {    
$_SESSION['CocaPayPayza'] = 0; 
}
...
if (@$_REQUEST['hourly']) {
$_SESSION['CocaPayPayza'] = 0;   
header("Location: cocapaypayza.php");

}

if (@$_REQUEST['daily']) {
$_SESSION['CocaPayPayza'] = 1;   
header("Location: cocapaypayza.php");
...
<script src="../realorders/real15.js"></script>
...
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="b15" onclick="buy(this.id)" class="button center">Yes</a></div>
...
<h1 style="color: #c92d2d; margin: 10px;" class="tcenter">You really want to create the order?</h1>
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="s15" onclick="sell(this.id)" class="button center">Yes</a></div>
...
<div id="orders-stats">
<div class="orderStats">Last Price: <strong>1.0939 CocaPay</strong></div>
<div class="orderStats">Low: <strong>1.0801 CocaPay</strong></div>
<div class="orderStats">High: <strong>1.108 CocaPay</strong></div>
<div class="clear"></div>
<div class="orderStats">Volume: <strong>41665 Payza / 45464 CocaPay</strong></div>
...
<li class="pairs-selected">CocaPay/Payza<span style="display:block">0.364</span></li>
...
<h1 id="n0">CocaPay/Payza</h1></div>
<form action="cocapaypayza.php" method="post">
...
<h1>Buy CocaPay</h1>
<input id="pair" value="20" type="hidden">
<div class="inblock order_header">
<div style="width: 130px; float: left;">Your balance:
<p><a href="javascript:void(0)" onclick="a_calc(20)"><b><span id="cur1" class="money_Payza">0</span><input id="cur1_full" value="0" type="hidden"> Payza</b></a></p>
</div>
...
<p class="gray" style="text-align: right;">Total: 80069.28 CocaPay</p>
...
$red = mysql_query("SELECT * FROM ordersCocaPayPayza WHERE action='0' AND type='sell' AND time < '$date'");
...
$red = mysql_query("SELECT * FROM realCocaPayPayza WHERE action='0' AND type='sell' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
<p class="gray" style="text-align: right;">Total: 52766.76 Payza</p>
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersCocaPayPayza WHERE action='0' AND type='buy' AND time < '$date'");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$red = mysql_query("SELECT * FROM realCocaPayPayza WHERE action='0' AND type='buy' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$user = $_SESSION['user'];
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM realCocaPayPayza WHERE action='0' AND user='$user' AND time < '$date' ORDER BY time DESC");
while ($row = mysql_fetch_array($red)){  
$price = $row['price'];
$amount = $row['amount'];
$total = $row['total'];
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersCocaPayPayza WHERE action='1' AND time2 <= '$date' ORDER BY time2 DESC");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
...
$red = mysql_query("SELECT * FROM realCocaPayPayza WHERE action='1'");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
