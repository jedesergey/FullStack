...
session_start();
if (!isset($_SESSION['CocaPayPerfectMoney'])) {    
$_SESSION['CocaPayPerfectMoney'] = 0; 
}
...
if($_SESSION['CocaPayPerfectMoney'] == 1){
$dis2 = 'disabled="disabled"'; 
$ch1 = 'CocaPayPerfectMoney';
$interval = '1 days';
$format = '%b %e';
$mintime = $minday; 
$maxtime = $maxday;    
...
$ch1 = 'ohlcordersCocaPayPerfectMoney';
$interval = '1 hours';
$format = '%R'; 
...
if (@$_REQUEST['hourly']) {
$_SESSION['CocaPayPerfectMoney'] = 0;    
header("Location: cocapayperfectmoney.php");

}

if (@$_REQUEST['daily']) {
$_SESSION['CocaPayPerfectMoney'] = 1;    
header("Location: cocapayperfectmoney.php");
...
<img src='../images/1px.png' class='main-s main-s-dollar_small' alt='dollar_small'/> <b class='money_CocaPay'>0</b> CocaPay
</div>
...
<title>CocaPay/PerfectMoney</title>
<meta name="robots" content="index, follow">
<meta name="keywords" content="Monero, обмен Monero, биржа Monero, обменник Monero, Bitcoin, биткоин, обмен биткоин, биржа биткоин, обменник биткоин, покупка Monero, продажа Monero, покупка bitcoin, продажа bitcoin, биржа bitcoin, обменник bitcoin">
<meta name="description" content="Monero-E">
<meta charset="utf-8">
...
<script src="../realorders/real11.js"></script>
...
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="b11" onclick="buy(this.id)" class="button center">Yes</a></div>
...
<h1 style="color: #c92d2d; margin: 10px;" class="tcenter">You really want to create the order?</h1>
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="s11" onclick="sell(this.id)" class="button center">Yes</a></div>
...
<div id="orders-stats">
<div class="orderStats">Last Price: <strong>3.0093 CocaPay</strong></div>
<div class="orderStats">Low: <strong>2.9731 CocaPay</strong></div>
<div class="orderStats">High: <strong>3.0759 CocaPay</strong></div>
<div class="clear"></div>
<div class="orderStats">Volume: <strong>78227 PerfectMoney / 235630 CocaPay</strong></div>
<div class="orderStats">Server Time: <strong>29.10.15 18:41</strong></div>
<div class="clear"></div>
</div>
</div>
<? echo $noauth; ?>
...
<li class="pairs-selected">CocaPay/PerfectMoney<span style="display:block">0.364</span></li>
...
<h1 id="n0">CocaPay/PerfectMoney</h1></div>
<form action="cocapayperfectmoney.php" method="post">
...
<h1>Buy CocaPay</h1>
<input id="pair" value="14" type="hidden">
<div class="inblock order_header">
<div style="width: 130px; float: left;">Your balance:
<p><a href="javascript:void(0)" onclick="a_calc(14)"><b><span id="cur1" class="money_PerfectMoney">0</span><input id="cur1_full" value="0" type="hidden"> PerfectMoney</b></a></p>
</div>
...
<td>
<input id="b_price" style="width: 110px;" maxlength="8" value="0"  type="text"> PerfectMoney</td>
</tr>
<tr>
<td>Total:</td>
...
<p class="gray" style="text-align: right;">Total: 755009.43 CocaPay</p>
...
$red = mysql_query("SELECT * FROM ordersCocaPayPerfectMoney WHERE action='0' AND type='sell' AND time < '$date'");
...
$red = mysql_query("SELECT * FROM realCocaPayPerfectMoney WHERE action='0' AND type='sell' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
<div class="inblock order_header">
<div style="width: 130px; float: left;">Your balance:
<p><a href="javascript:void(0)" onclick='all_money("cur2_full", "s_Monero")'><b><span id="cur2" class="money_CocaPay">0</span><input id="cur2_full" value="0" type="hidden"> CocaPay</b></a></p>
</div>
...
<td>Price per CocaPay:</td>
<td>
<input id="s_price" style="width: 110px;" maxlength="8" value="0"  type="text"> PerfectMoney</td>
</tr>
<tr>
<td>Total:</td>
...
<p class="gray" style="text-align: right;">Total: 2665122.94 PerfectMoney</p>
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersCocaPayPerfectMoney WHERE action='0' AND type='buy' AND time < '$date'");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$red = mysql_query("SELECT * FROM realCocaPayPerfectMoney WHERE action='0' AND type='buy' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$user = $_SESSION['user'];
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM realCocaPayPerfectMoney WHERE action='0' AND user='$user' AND time < '$date' ORDER BY time DESC");
while ($row = mysql_fetch_array($red)){  
$price = $row['price'];
$amount = $row['amount'];
$total = $row['total'];
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersCocaPayPerfectMoney WHERE action='1' AND time2 <= '$date' ORDER BY time2 DESC");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
...
$red = mysql_query("SELECT * FROM realCocaPayPerfectMoney WHERE action='1'");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
