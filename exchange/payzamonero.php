...
session_start();
if (!isset($_SESSION['PayzaMonero'])) {    
$_SESSION['PayzaMonero'] = 0; 
}
...
if (@$_REQUEST['hourly']) {
$_SESSION['PayzaMonero'] = 0;   
header("Location: payzamonero.php");

}

if (@$_REQUEST['daily']) {
$_SESSION['PayzaMonero'] = 1;    
header("Location: payzamonero.php");
...
<script src="../realorders/real14.js"></script>
...
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="b14" onclick="buy(this.id)" class="button center">Yes</a></div>
...
<h1 style="color: #c92d2d; margin: 10px;" class="tcenter">You really want to create the order?</h1>
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="s14" onclick="sell(this.id)" class="button center">Yes</a></div>
...
<div id="orders-stats">
<div class="orderStats">Last Price: <strong>0.0097 Monero</strong></div>
<div class="orderStats">Low: <strong>0.0097 Monero</strong></div>
<div class="orderStats">High: <strong>0.0103 Monero</strong></div>
<div class="clear"></div>
<div class="orderStats">Volume: <strong>165116 Payza / 1654 Monero</strong></div>
<div class="orderStats">Server Time: <strong>29.10.15 18:40</strong></div>
<div class="clear"></div>
</div>
</div>
<? echo $noauth; ?>
...
<li class="pairs-selected">Payza/Monero<span style="display:block">0.364</span></li>
...
<h1 id="n0">Payza/Monero</h1></div>

<form action="payzamonero.php" method="post">
...
<h1>Buy Payza</h1>
<input id="pair" value="10" type="hidden">
<div class="inblock order_header">
<div style="width: 130px; float: left;">Your balance:
<p><a href="javascript:void(0)" onclick="a_calc(10)"><b><span id="cur1" class="money_Monero">0</span><input id="cur1_full" value="0" type="hidden"> Monero</b></a></p>
</div>
...
<tbody>
<tr>
<td style="width: 150px;">Amount Payza:</td>
<td style="width: 153px;">
<input id="b_Skrill" style="width: 110px;" maxlength="25" value="0" onkeyup="sendbuy(this.value)" type="text">
</td>
</tr>
<tr class="price-row">
<td>Price per Payza:</td>
<td>
<input id="b_price" style="width: 110px;" maxlength="7" value="0"  type="text"> Monero</td>
</tr>
<tr>
<td>Total:</td>
...
<a href="javascript:void(0)" onclick="noticebuy()" class="button" style="float: right;">Buy Payza</a>
...
<p class="gray" style="text-align: right;">Total: 535044.23 Payza</p>
...
$red = mysql_query("SELECT * FROM ordersPayzaMonero WHERE action='0' AND type='sell' AND time < '$date'");
...
$red = mysql_query("SELECT * FROM realPayzaMonero WHERE action='0' AND type='sell' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
<div class="inblock order_header">
<div style="width: 130px; float: left;">Your balance:
<p><a href="javascript:void(0)" onclick='all_money("cur2_full", "s_Monero")'><b><span id="cur2" class="money_Payza">0</span><input id="cur2_full" value="0" type="hidden"> Payza</b></a></p>
</div>
...
<td>Price per Payza:</td>
<td>
<input id="s_price" style="width: 110px;" maxlength="7" value="0"  type="text"> Monero</td>
</tr>
<tr>
<td>Total:</td>
...
<tr>
<td colspan="2">
<a href="javascript:void(0)" onclick='ex_calculate("sell",10)' class="button" style="float: left;">Calculate</a>
<div id="s_loading" class="order_calc_load"></div>
<a href="javascript:void(0)" onclick="noticesell()" class="button" style="float: right;">Sell Payza</a>
...
<p class="gray" style="text-align: right;">Total: 1655.15 Monero</p>
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersPayzaMonero WHERE action='0' AND type='buy' AND time < '$date'");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$red = mysql_query("SELECT * FROM realPayzaMonero WHERE action='0' AND type='buy' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$user = $_SESSION['user'];
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM realPayzaMonero WHERE action='0' AND user='$user' AND time < '$date' ORDER BY time DESC");
while ($row = mysql_fetch_array($red)){  
$price = $row['price'];
$amount = $row['amount'];
$total = $row['total'];
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersPayzaMonero WHERE action='1' AND time2 <= '$date' ORDER BY time2 DESC");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
...
$red = mysql_query("SELECT * FROM realPayzaMonero WHERE action='1'");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
