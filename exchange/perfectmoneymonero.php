...
session_start();
if (!isset($_SESSION['PerfectMoneyMonero'])) {    
$_SESSION['PerfectMoneyMonero'] = 0; 
}
...
if($_SESSION['PerfectMoneyMonero'] == 1){
$dis2 = 'disabled="disabled"'; 
$ch1 = 'PerfectMoneyMonero';
$interval = '1 days';
$format = '%b %e'; 
$mintime = $minday; 
$maxtime = $maxday;    
...
$ch1 = 'ohlcordersPerfectMoneyMonero';
$interval = '1 hours';
$format = '%R'; 
...
if (@$_REQUEST['hourly']) {
$_SESSION['PerfectMoneyMonero'] = 0;    
header("Location: perfectmoneymonero.php");

}

if (@$_REQUEST['daily']) {
$_SESSION['PerfectMoneyMonero'] = 1;    
header("Location: perfectmoneymonero.php");
...
<title>PerfectMoney/Monero</title>
...
<script src="../realorders/real10.js"></script>
...
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="b10" onclick="buy(this.id)" class="button center">Yes</a></div>
...
<h1 style="color: #c92d2d; margin: 10px;" class="tcenter">You really want to create the order?</h1>
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="s10" onclick="sell(this.id)" class="button center">Yes</a></div>
...
<div id="orders-stats">
<div class="orderStats">Last Price: <strong>0.0011 Monero</strong></div>
<div class="orderStats">Low: <strong>0.0011 Monero</strong></div>
<div class="orderStats">High: <strong>0.0012 Monero</strong></div>
<div class="clear"></div>
<div class="orderStats">Volume: <strong>37956 PerfectMoney / 44 Monero</strong></div>
<div class="orderStats">Server Time: <strong>29.10.15 18:42</strong></div>
<div class="clear"></div>
</div>
</div>
<? echo $noauth; ?>
...
<li class="pairs-selected">PerfectMoney/Monero<span style="display:block">0.364</span></li>
...
<h1 id="n0">PerfectMoney/Monero</h1></div>
<form action="perfectmoneymonero.php" method="post">
...
<input id="pair" value="13" type="hidden">
<div class="inblock order_header">
<div style="width: 130px; float: left;">Your balance:
<p><a href="javascript:void(0)" onclick="a_calc(13)"><b><span id="cur1" class="money_Monero">0</span><input id="cur1_full" value="0" type="hidden"> Monero</b></a></p>
</div>
...
<p class="gray" style="text-align: right;">Total: 623643.36 PerfectMoney</p>
...
$red = mysql_query("SELECT * FROM ordersPerfectMoneyMonero WHERE action='0' AND type='sell' AND time < '$date'");
...
$red = mysql_query("SELECT * FROM realPerfectMoneyMonero WHERE action='0' AND type='sell' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
<p class="gray" style="text-align: right;">Total: 155.93 Monero</p>
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersPerfectMoneyMonero WHERE action='0' AND type='buy' AND time < '$date'");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$red = mysql_query("SELECT * FROM realPerfectMoneyMonero WHERE action='0' AND type='buy' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$user = $_SESSION['user'];
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM realPerfectMoneyMonero WHERE action='0' AND user='$user' AND time < '$date' ORDER BY time DESC");
while ($row = mysql_fetch_array($red)){  
$price = $row['price'];
$amount = $row['amount'];
$total = $row['total'];
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersPerfectMoneyMonero WHERE action='1' AND time2 <= '$date' ORDER BY time2 DESC");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
...
$red = mysql_query("SELECT * FROM realPerfectMoneyMonero WHERE action='1'");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
