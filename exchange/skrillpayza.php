...
session_start();
if (!isset($_SESSION['SkrillPayza'])) {    
$_SESSION['SkrillPayza'] = 0; 
}
...
if (@$_REQUEST['hourly']) {
$_SESSION['SkrillPayza'] = 0;    
header("Location: skrillpayza.php");

}

if (@$_REQUEST['daily']) {
$_SESSION['SkrillPayza'] = 1;   
header("Location: skrillpayza.php");
...
<title>Skrill/Payza</title>
<meta name="robots" content="index, follow">
<meta name="keywords" content="Skrill, обмен Skrill, биржа Skrill, обменник Skrill, Bitcoin, биткоин, обмен биткоин, биржа биткоин, обменник биткоин, покупка Skrill, продажа Skrill, покупка bitcoin, продажа bitcoin, биржа bitcoin, обменник bitcoin">
...
<script type='text/javascript' src='../realorders/real2.js'></script>
...
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="b2" onclick="buy(this.id)" class="button center">Yes</a></div>
...
<h1 style="color: #c92d2d; margin: 10px;" class="tcenter">You really want to create the order?</h1>
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="s2" onclick="sell(this.id)" class="button center">Yes</a></div>
...
<div id="orders-stats">
<div class="orderStats">Last Price: <strong>282.9498 Payza</strong></div>
<div class="orderStats">Low: <strong>266.1757 Payza</strong></div>
<div class="orderStats">High: <strong>284.29 Payza</strong></div>
<div class="clear"></div>
<div class="orderStats">Volume: <strong>282 Skrill / 77920 Payza</strong></div>
<div class="orderStats">Server Time: <strong>29.10.15 18:38</strong></div>
<div class="clear"></div>
</div>
</div>
<? echo $noauth; ?>
...
<li class="pairs-selected">Skrill/Payza<span style="display:block">0.364</span></li>
...
<h1 id="n0">Skrill/Payza</h1></div>
<form action="skrillpayza.php" method="post">
...
<p><a href="javascript:void(0)" onclick="a_calc(19)"><b><span id="cur1" class="money_Payza">0</span><input id="cur1_full" value="0" type="hidden"> Payza</b></a></p>
</div>
<div style="width: 130px; float: right;">Lowest ask Price
<p><b><span id="min_price">0</span> Payza</b></p>
</div>
<div class="clear"></div>
</div>
...
<td>
<input id="b_price" style="width: 110px;" maxlength="8" value="0"  type="text"> Payza</td>
</tr>
<tr>
<td>Total:</td>
<td><b id="b_all">0</b> <b>Payza</b></td>
</tr>
<tr>
<td>Fee:</td>
<td><b id="b_fee">0</b> <b>Payza</b></td>
</tr>
...
$red = mysql_query("SELECT * FROM ordersSkrillPayza WHERE action='0' AND type='sell' AND time < '$date'");
...
$red = mysql_query("SELECT * FROM realSkrillPayza WHERE action='0' AND type='sell' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
<p><b><span id="max_price">0</span> Payza</b></p>
</div>
<div class="clear"></div>
</div>
<table class="inblock tabla2" style="width: 100%;">
...
<td>Price per Skrill:</td>
<td>
<input id="s_price" style="width: 110px;" maxlength="8" value="0"  type="text"> Payza</td>
</tr>
<tr>
<td>Total:</td>
<td><b id="s_all">0</b> <b>Payza</b></td>
</tr>
<tr>
<td>Fee:</td>
<td><b id="s_fee">0</b> <b>Payza</b></td>
</tr>
...
<p class="gray" style="text-align: right;">Total: 215365.44 Payza</p>
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersSkrillPayza WHERE action='0' AND type='buy' AND time < '$date'");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$red = mysql_query("SELECT * FROM realSkrillPayza WHERE action='0' AND type='buy' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$user = $_SESSION['user'];
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM realSkrillPayza WHERE action='0' AND user='$user' AND time < '$date' ORDER BY time DESC");
while ($row = mysql_fetch_array($red)){  
$price = $row['price'];
$amount = $row['amount'];
$total = $row['total'];
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersSkrillPayza WHERE action='1' AND time2 <= '$date' ORDER BY time2 DESC");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
...
$red = mysql_query("SELECT * FROM realSkrillPayza WHERE action='1'");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
