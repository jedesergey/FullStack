...
session_start();
if (!isset($_SESSION['CocaPayPayeer'])) {    
$_SESSION['CocaPayPayeer'] = 0; 
}
...
if (@$_REQUEST['hourly']) {
$_SESSION['CocaPayPayeer'] = 0;    
header("Location: cocapaypayeer.php");

}

if (@$_REQUEST['daily']) {
$_SESSION['CocaPayPayeer'] = 1;    
header("Location: cocapaypayeer.php");
...
<script src="../realorders/real18.js"></script>
...
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="b18" onclick="buy(this.id)" class="button center">Yes</a></div>
...
<h1 style="color: #c92d2d; margin: 10px;" class="tcenter">You really want to create the order?</h1>
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="s18" onclick="sell(this.id)" class="button center">Yes</a></div>
...
<div id="orders-stats">
<div class="orderStats">Last Price: <strong>69.0834 CocaPay</strong></div>
<div class="orderStats">Low: <strong>67.83 CocaPay</strong></div>
<div class="orderStats">High: <strong>70.0985 CocaPay</strong></div>
<div class="clear"></div>
<div class="orderStats">Volume: <strong>1890 Payeer / 130365 CocaPay</strong></div>
...
<li class="pairs-selected">CocaPay/Payeer<span style="display:block">0.364</span></li>
...
<h1 id="n0">CocaPay/Payeer</h1></div>
<form action="cocapaypayeer.php" method="post">
...
<h1>Buy CocaPay</h1>
<input id="pair" value="32" type="hidden">
<div class="inblock order_header">
<div style="width: 130px; float: left;">Your balance:
<p><a href="javascript:void(0)" onclick="a_calc(32)"><b><span id="cur1" class="money_Payeer">0</span><input id="cur1_full" value="0" type="hidden"> Payeer</b></a></p>
</div>
...
<input id="b_Skrill" onkeyup="sendbuy(this.value)" style="width: 110px;" maxlength="25" value="0"  type="text">
</td>
</tr>
<tr class="price-row">
...
<p class="gray" style="text-align: right;">Total: 6550.23 CocaPay</p>
...
$red = mysql_query("SELECT * FROM ordersCocaPayPayeer WHERE action='0' AND type='sell' AND time < '$date'");
...
$red = mysql_query("SELECT * FROM realCocaPayPayeer WHERE action='0' AND type='sell' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
<p class="gray" style="text-align: right;">Total: 223378.21 Payeer</p>
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersCocaPayPayeer WHERE action='0' AND type='buy' AND time < '$date'");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$red = mysql_query("SELECT * FROM realCocaPayPayeer WHERE action='0' AND type='buy' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$user = $_SESSION['user'];
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM realCocaPayPayeer WHERE action='0' AND user='$user' AND time < '$date' ORDER BY time DESC");
while ($row = mysql_fetch_array($red)){  
$price = $row['price'];
$amount = $row['amount'];
$total = $row['total'];
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersCocaPayPayeer WHERE action='1' AND time2 <= '$date' ORDER BY time2 DESC");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
...
$red = mysql_query("SELECT * FROM realCocaPayPayeer WHERE action='1'");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
