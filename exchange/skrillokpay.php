...
session_start();
if (!isset($_SESSION['SkrillOKPay'])) {    
$_SESSION['SkrillOKPay'] = 0; 
}
...
if (@$_REQUEST['hourly']) {
$_SESSION['SkrillOKPay'] = 0;    
header("Location: skrillokpay.php");

}

if (@$_REQUEST['daily']) {
$_SESSION['SkrillOKPay'] = 1;    
header("Location: skrillokpay.php");
...
<img src='../images/1px.png' class='main-s main-s-dollar_small' alt='dollar_small'/> <b class='money_Skrill'>0</b> Skrill
</div>
...
<script src="../realorders/real3.js"></script>
...
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="b3" onclick="buy(this.id)" class="button center">Yes</a></div>
...
<h1 style="color: #c92d2d; margin: 10px;" class="tcenter">You really want to create the order?</h1>
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="s3" onclick="sell(this.id)" class="button center">Yes</a></div>
...
<div id="orders-stats">
<div class="orderStats">Last Price: <strong>62.8 OKPay</strong></div>
<div class="orderStats">Low: <strong>62.7801 OKPay</strong></div>
<div class="orderStats">High: <strong>63.49 OKPay</strong></div>
<div class="clear"></div>
<div class="orderStats">Volume: <strong>36057 Skrill / 2275369 OKPay</strong></div>
...
<li class="pairs-selected">Skrill/OKPay<span style="display:block">0.364</span></li>
...
<h1 id="n0">Skrill/OKPay</h1></div>

<form action="skrillokpay.php" method="post">
...
<h1>Buy Skrill</h1>
<input id="pair" value="18" type="hidden">
<div class="inblock order_header">
<div style="width: 130px; float: left;">Your balance:
<p><a href="javascript:void(0)" onclick="a_calc(18)"><b><span id="cur1" class="money_OKPay">0</span><input id="cur1_full" value="0" type="hidden"> OKPay</b></a></p>
</div>
...
<td>
<input id="b_price" style="width: 110px;" maxlength="8" value="0"  type="text"> OKPay</td>
</tr>
<tr>
<td>Total:</td>
...
<p class="gray" style="text-align: right;">Total: 54281.57 Skrill</p>
...
$red = mysql_query("SELECT * FROM ordersSkrillOKPay WHERE action='0' AND type='sell' AND time < '$date'");
...
$red = mysql_query("SELECT * FROM realSkrillOKPay WHERE action='0' AND type='sell' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
<td>Price per Skrill:</td>
<td>
<input id="s_price" style="width: 110px;" maxlength="8" value="0"  type="text"> OKPay</td>
</tr>
<tr>
<td>Total:</td>
...
<p class="gray" style="text-align: right;">Total: 4943790.35 OKPay</p>
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersSkrillOKPay WHERE action='0' AND type='buy' AND time < '$date'");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$red = mysql_query("SELECT * FROM realSkrillOKPay WHERE action='0' AND type='buy' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$user = $_SESSION['user'];
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM realSkrillOKPay WHERE action='0' AND user='$user' AND time < '$date' ORDER BY time DESC");
while ($row = mysql_fetch_array($red)){  
$price = $row['price'];
$amount = $row['amount'];
$total = $row['total'];
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersSkrillOKPay WHERE action='1' AND time2 <= '$date' ORDER BY time2 DESC");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
...
$red = mysql_query("SELECT * FROM realSkrillOKPay WHERE action='1'");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
