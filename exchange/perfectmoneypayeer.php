...
session_start();
if (!isset($_SESSION['PerfectMoneyPayeer'])) {    
$_SESSION['PerfectMoneyPayeer'] = 0; 
}
...
if($_SESSION['PerfectMoneyPayeer'] == 1){
$dis2 = 'disabled="disabled"'; 
$ch1 = 'PerfectMoneyPayeer';
$interval = '1 days';
$format = '%b %e';
$mintime = $minday; 
$maxtime = $maxday;    
...
$ch1 = 'ohlcordersPerfectMoneyPayeer';
$interval = '1 hours';
$format = '%R';
...
if (@$_REQUEST['hourly']) {
$_SESSION['PerfectMoneyPayeer'] = 0;    
header("Location: perfectmoneypayeer.php");

}

if (@$_REQUEST['daily']) {
$_SESSION['PerfectMoneyPayeer'] = 1;    
header("Location: perfectmoneypayeer.php");
...
<img src='../images/1px.png' class='main-s main-s-bitcoin_small' alt='bitcoin_small'/> <b class='money_Payeer'>0</b> Payeer
</div>
<div class='clear'></div>
</div>
</div></div>
...
<title>PerfectMoney/Payeer</title>
...
<script src="../realorders/real9.js"></script>
...
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="b9" onclick="buy(this.id)" class="button center">Yes</a></div>
...
<h1 style="color: #c92d2d; margin: 10px;" class="tcenter">You really want to create the order?</h1>
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="s9" onclick="sell(this.id)" class="button center">Yes</a></div>
...
<div id="orders-stats">
<div class="orderStats">Last Price: <strong>0.0028 Payeer</strong></div>
<div class="orderStats">Low: <strong>0.0028 Payeer</strong></div>
<div class="orderStats">High: <strong>0.0029 Payeer</strong></div>
<div class="clear"></div>
<div class="orderStats">Volume: <strong>2770 PerfectMoney / 8 Payeer</strong></div>
...
<a href="https://Payeer-e.com/setlocale/en"><img src="/images/1px.png" class="main-s main-s-en" alt="en"></a>
<a href="https://Payeer-e.com/setlocale/ru"><img src="/images/1px.png" class="main-s main-s-ru" alt="ru"></a>
<a href="https://Payeer-e.com/setlocale/cn"><img src="/images/1px.png" class="main-s main-s-cn" alt="cn"></a>
</div> -->
</div>
</div>
</div>
...
<li class="pairs-selected">PerfectMoney/Payeer<span style="display:block">0.364</span></li>
...
<h1 id="n0">PerfectMoney/Payeer</h1></div>

<form action="perfectmoneypayeer.php" method="post">
...
<input id="pair" value="22" type="hidden">
<div class="inblock order_header">
<div style="width: 130px; float: left;">Your balance:
<p><a href="javascript:void(0)" onclick="a_calc(22)"><b><span id="cur1" class="money_Payeer">0</span><input id="cur1_full" value="0" type="hidden"> Payeer</b></a></p>
</div>
...
<p class="gray" style="text-align: right;">Total: 50805.38 PerfectMoney</p>
...
$red = mysql_query("SELECT * FROM ordersPerfectMoneyPayeer WHERE action='0' AND type='sell' AND time < '$date'");
...
$red = mysql_query("SELECT * FROM realPerfectMoneyPayeer WHERE action='0' AND type='sell' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
<div class="inblock order_header">
<div style="width: 130px; float: left;">Your balance:
<p><a href="javascript:void(0)" onclick='all_money("cur2_full", "s_Payeer")'><b><span id="cur2" class="money_PerfectMoney">0</span><input id="cur2_full" value="0" type="hidden"> PerfectMoney</b></a></p>
</div>
...
<p class="gray" style="text-align: right;">Total: 103 Payeer</p>
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersPerfectMoneyPayeer WHERE action='0' AND type='buy' AND time < '$date'");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$red = mysql_query("SELECT * FROM realPerfectMoneyPayeer WHERE action='0' AND type='buy' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$user = $_SESSION['user'];
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM realPerfectMoneyPayeer WHERE action='0' AND user='$user' AND time < '$date' ORDER BY time DESC");
while ($row = mysql_fetch_array($red)){  
$price = $row['price'];
$amount = $row['amount'];
$total = $row['total'];
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersPerfectMoneyPayeer WHERE action='1' AND time2 <= '$date' ORDER BY time2 DESC");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
...
$red = mysql_query("SELECT * FROM realPerfectMoneyPayeer WHERE action='1'");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
...
<p class="copyrights">Copyright © 2011-2015 Payeer-e.com. All rights reserved.</p>
<p class="copyrights">
<a href="https://Payeer-e.com/api/3/docs">Public API</a> |
<a href="https://Payeer-e.com/tapi/docs">Trade API</a> |
