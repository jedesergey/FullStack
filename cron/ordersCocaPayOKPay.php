...
$f = mysql_query("SELECT * FROM ordersCocaPayOKPay WHERE YEAR(time2) = '$viewtimes1' AND MONTH(time2) = '$viewtimes2' AND DAY(time2) = '$viewtimes3' AND HOUR(time2) = '$viewtimes4' AND action = '1' ", $link);
$row8 = mysql_fetch_array($f);
    
$c = mysql_query("SELECT * FROM realCocaPayOKPay WHERE YEAR(time2) = '$viewtimes1' AND MONTH(time2) = '$viewtimes2' AND DAY(time2) = '$viewtimes3' AND HOUR(time2) = '$viewtimes4' AND action = '1' ", $link);
$row3 = mysql_fetch_array($c);    

if ($row8 OR $row3){    
    
    
$c = mysql_query("SELECT * FROM ordersCocaPayOKPay WHERE YEAR(time2) = '$viewtimes1' AND MONTH(time2) = '$viewtimes2' AND DAY(time2) = '$viewtimes3' AND HOUR(time2) = '$viewtimes4' AND action = '1' ", $link);
...
$i=1;
$stringfile2[0] = 'ohlcordersCocaPayOKPay'.' = [';
...
$stringfile2[$i] = '["'.$row1.'", '.$row3.', '.$row2.', '.$row5.', '.$row4.' ],  ' . "\n";
$i++;	
	
}
$i++;
$stringfile2[$i] = '];';    
file_put_contents('ordersCocaPayOKPay.js', $stringfile2);
