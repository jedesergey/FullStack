...
session_start();
if (!isset($_SESSION['PerfectMoneyOKPay'])) {    
$_SESSION['PerfectMoneyOKPay'] = 0; 
}
...
if($_SESSION['PerfectMoneyOKPay'] == 1){
$dis2 = 'disabled="disabled"'; 
$ch1 = 'PerfectMoneyOKPay';
$interval = '1 days';
$format = '%b %e';
$mintime = $minday; 
$maxtime = $maxday;    
...
$ch1 = 'ohlcordersPerfectMoneyOKPay';
$interval = '1 hours';
$format = '%R';
...
if (@$_REQUEST['hourly']) {
$_SESSION['PerfectMoneyOKPay'] = 0;   
header("Location: perfectmoneyokpay.php");

}

if (@$_REQUEST['daily']) {
$_SESSION['PerfectMoneyOKPay'] = 1;    
header("Location: perfectmoneyokpay.php");
...
<img src='../images/1px.png' class='main-s main-s-dollar_small' alt='dollar_small'/> <b class='money_OKPay'>0</b> OKPay
</div>
...
<title>PerfectMoney/OKPay</title>
...
<script src="../realorders/real8.js"></script>
...
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="b8" onclick="buy(this.id)" class="button center">Yes</a></div>
...
<h1 style="color: #c92d2d; margin: 10px;" class="tcenter">You really want to create the order?</h1>
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="s8" onclick="sell(this.id)" class="button center">Yes</a></div>
...
<div id="orders-stats">
<div class="orderStats">Last Price: <strong>0.353 OKPay</strong></div>
<div class="orderStats">Low: <strong>0.345 OKPay</strong></div>
<div class="orderStats">High: <strong>0.3569 OKPay</strong></div>
<div class="clear"></div>
<div class="orderStats">Volume: <strong>14125 PerfectMoney / 4957 OKPay</strong></div>
...
<li class="pairs-selected">PerfectMoney/OKPay<span style="display:block">0.364</span></li>
...
<h1 id="n0">PerfectMoney/OKPay</h1></div>
<form action="perfectmoneyokpay.php" method="post">
...
<input id="pair" value="28" type="hidden">
<div class="inblock order_header">
<div style="width: 130px; float: left;">Your balance:
<p><a href="javascript:void(0)" onclick="a_calc(28)"><b><span id="cur1" class="money_OKPay">0</span><input id="cur1_full" value="0" type="hidden"> OKPay</b></a></p>
</div>
...
<p class="gray" style="text-align: right;">Total: 746409.24 PerfectMoney</p>
...
$red = mysql_query("SELECT * FROM ordersPerfectMoneyOKPay WHERE action='0' AND type='sell' AND time < '$date'");
...
$red = mysql_query("SELECT * FROM realPerfectMoneyOKPay WHERE action='0' AND type='sell' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
<p class="gray" style="text-align: right;">Total: 26843.7 OKPay</p>
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersPerfectMoneyOKPay WHERE action='0' AND type='buy' AND time < '$date'");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$red = mysql_query("SELECT * FROM realPerfectMoneyOKPay WHERE action='0' AND type='buy' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$user = $_SESSION['user'];
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM realPerfectMoneyOKPay WHERE action='0' AND user='$user' AND time < '$date' ORDER BY time DESC");
while ($row = mysql_fetch_array($red)){  
$price = $row['price'];
$amount = $row['amount'];
$total = $row['total'];
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersPerfectMoneyOKPay WHERE action='1' AND time2 <= '$date' ORDER BY time2 DESC");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
...
$red = mysql_query("SELECT * FROM realPerfectMoneyOKPay WHERE action='1'");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
