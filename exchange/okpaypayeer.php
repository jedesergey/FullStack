...
session_start();
if (!isset($_SESSION['OKPayPayeer'])) {    
$_SESSION['OKPayPayeer'] = 0; 
}
...
if (@$_REQUEST['hourly']) {
$_SESSION['OKPayPayeer'] = 0;    
header("Location: okpaypayeer.php");

}

if (@$_REQUEST['daily']) {
$_SESSION['OKPayPayeer'] = 1;   
header("Location: okpaypayeer.php");
...
<img src='../images/1px.png' class='main-s main-s-dollar_small' alt='dollar_small'/> <b class='money_Payeer'>0</b> Payeer
</div>
...
<script src="../realorders/real16.js"></script>
...
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="b16" onclick="buy(this.id)" class="button center">Yes</a></div>
...
<h1 style="color: #c92d2d; margin: 10px;" class="tcenter">You really want to create the order?</h1>
<div style="float:left; margin-left: 20px; width: 220px;"><a href="javascript:void(0)" id="s16" onclick="sell(this.id)" class="button center">Yes</a></div>
...
<div id="orders-stats">
<div class="orderStats">Last Price: <strong>307.988 Payeer</strong></div>
<div class="orderStats">Low: <strong>294.1 Payeer</strong></div>
<div class="orderStats">High: <strong>308 Payeer</strong></div>
<div class="clear"></div>
<div class="orderStats">Volume: <strong>10943 OKPay / 3281239 Payeer</strong></div>
...
<li class="pairs-selected">OKPay/Payeer<span style="display:block">0.364</span></li>
...
<h1 id="n0">OKPay/Payeer</h1></div>
<form action="okpaypayeer.php" method="post">
...
<p><a href="javascript:void(0)" onclick="a_calc(1)"><b><span id="cur1" class="money_Payeer">0</span><input id="cur1_full" value="0" type="hidden"> Payeer</b></a></p>
</div>
...
<tbody>
<tr>
<td style="width: 150px;">Amount OKPay:</td>
<td style="width: 153px;">
<input id="b_Skrill" style="width: 110px;" maxlength="25" value="0" onkeyup="sendbuy(this.value)" type="text">
</td>
</tr>
<tr class="price-row">
<td>Price per OKPay:</td>
<td>
<input id="b_price" style="width: 110px;" maxlength="7" value="0"  type="text"> Payeer</td>
</tr>
<tr>
<td>Total:</td>
...
<a href="javascript:void(0)" onclick="noticebuy()" class="button" style="float: right;">Buy OKPay</a>
...
<p class="gray" style="text-align: right;">Total: 6556.18 OKPay</p>
...
$red = mysql_query("SELECT * FROM ordersOKPayPayeer WHERE action='0' AND type='sell' AND time < '$date'");
...
$red = mysql_query("SELECT * FROM realOKPayPayeer WHERE action='0' AND type='sell' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
<div class="inblock order_header">
<div style="width: 130px; float: left;">Your balance:
<p><a href="javascript:void(0)" onclick='all_money("cur2_full", "s_OKPay")'><b><span id="cur2" class="money_OKPay">0</span><input id="cur2_full" value="0" type="hidden"> OKPay</b></a></p>
</div>
...
<td>Price per OKPay:</td>
<td>
<input id="s_price" style="width: 110px;" maxlength="7" value="0"  type="text"> Payeer</td>
</tr>
<tr>
<td>Total:</td>
...
<a href="javascript:void(0)" onclick="noticesell()" class="button" style="float: right;">Sell OKPay</a>
...
<p class="gray" style="text-align: right;">Total: 3796079.17 Payeer</p>
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersOKPayPayeer WHERE action='0' AND type='buy' AND time < '$date'");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$red = mysql_query("SELECT * FROM realOKPayPayeer WHERE action='0' AND type='buy' ");
while ($row = mysql_fetch_array($red)){  
$time = $row['time'];
$price = $row['price'];
$amount = $row['amount'];
...
$user = $_SESSION['user'];
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM realOKPayPayeer WHERE action='0' AND user='$user' AND time < '$date' ORDER BY time DESC");
while ($row = mysql_fetch_array($red)){  
$price = $row['price'];
$amount = $row['amount'];
$total = $row['total'];
...
$date = date("Y-m-d H:i:s");                                                 
$red = mysql_query("SELECT * FROM ordersOKPayPayeer WHERE action='1' AND time2 <= '$date' ORDER BY time2 DESC");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
...
$red = mysql_query("SELECT * FROM realOKPayPayeer WHERE action='1'");
while ($row = mysql_fetch_array($red)){  
$time2 = $row['time2'];
$type2 = $row['type2'];
$price2 = $row['price'];
